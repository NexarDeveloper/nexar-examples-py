...
`python supply_filter.py`

When the script is executed, the user is asked for a token input. Then the script will query the supply API for all available inputs for the filter declared in the main program. The returned available inputs are then listed. The user then is requested to enter an input by entering the index for the input they want. The script will loop until a valid index value is entered. Once an index value is entered, that is valid, the script will run a second query for the first 10 parts that have the users choice of input for that particular filter. It will then display the amount of hits, part name, category, manufacturer, MPN, average number of parts available, description, median price, currency, the filter attribute name and the display value. This filter can be renamed to another filter and the script will run the same query with a different filter. 